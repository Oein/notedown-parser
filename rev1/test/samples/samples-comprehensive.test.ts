...
describe("Sample Files Comprehensive Tests", () => {
  ...
  describe("LaTeX Sample Tests", () => {
    test("should parse and render LaTeX formulas", () => {
      ...
      function findLatexNodes(item: any): boolean {
...
탈출 문자를 활용해서 같은 문단 안에서 여러 줄바꿈을 할 수 있음`;
    ...
    test("should parse basic paragraph structure", () => {
      ...
      expect(paragraphs[0].content[0].content[0].text).toBe(
        "본문을 이렇게 적을 수 있음."
      );
      expect(paragraphs[0].content[1].content[0].text).toBe(
        "같은 문단에서 내려쓰기를 할 수 있음."
      );
    });
    ...
    test("should handle paragraph breaks correctly", () => {
      ...
      expect(paragraphs[1].content[0].content[0].text).toBe(
        "두번 내려쓰면 다음 문단으로 전환"
      );
      expect(paragraphs[2].content[0].content[0].text).toBe(
        "세번 내려 써도 두번 한것과 같은 효과임"
      );
    });
...
\`\`\``;
    ...
    test("should handle escaped code syntax", () => {
      ...
      const escapedBlock = codeBlocks.find((block) =>
...
탈출 문자로 탈출 가능`;
    ...
    test("should parse Korean text in collapse titles", () => {
      ...
      expect(title0 && "text" in title0 ? title0.text : null).toBe(
        "접을 수 있는 제목"
      );
      expect(title1 && "text" in title1 ? title1.text : null).toBe(
        "접을 수 있는 2제목"
      );
      expect(title2 && "text" in title2 ? title2.text : null).toBe(
        "접을 수 있는 3제목"
      );
    });
    ...
    test("should render collapse blocks to HTML", () => {
      ...
      expect(html).toContain(
        '<details class="notedown-collapse notedown-collapse-1"'
      );
      expect(html).toContain(
        '<details class="notedown-collapse notedown-collapse-2"'
      );
      expect(html).toContain(
        '<details class="notedown-collapse notedown-collapse-3"'
      );
