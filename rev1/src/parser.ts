      ...
      while (j < lines.length) {
        ...
        if (
          ...
          if (contentLines.length > 0) {
            ...
            // First detect if this content block is entirely a code block
            for (let lineIdx = 0; lineIdx < contentLines.length; lineIdx++) {
              const line = contentLines[lineIdx]?.trim() || "";
              if (!inCodeBlock && line.startsWith("```")) {
                inCodeBlock = true;
                codeBlockStart = lineIdx;
                codeBlockLang = line.slice(3).trim();
              } else if (inCodeBlock && line === "```") {
                // If the code block spans the entire content (with potential whitespace)
                if (codeBlockStart === 0 && lineIdx === contentLines.length - 1) {
                  // Special handling for complete code blocks
                  // Extract the code content with original indentation
                  const codeContent = contentLines
                    .slice(codeBlockStart + 1, lineIdx)
                    .join("\n");
                  
                  // Add code block directly to content_blocks
                  ...
                  // Skip normal content processing
                  inCodeBlock = false;
                  k = lineIdx + 1; // Move past this code block
            ...
            // Only parse as regular content if it's not a complete code block
