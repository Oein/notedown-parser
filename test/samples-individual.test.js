...
탈출 문자를 활용해서 같은 문단 안에서 여러 줄바꿈을 할 수 있음`;
        ...
        expect(result.content.filter((item) => item.type === "paragraph").length).toBeGreaterThan(2);
...
That's all!`;
        ...
        const hasTitle = (content) => {
            return content.some((item) => {
                if (item.type === "paragraph" && item.content) {
                    return item.content.some((c) => c.type === "title");
                }
                return false;
            });
        };
        ...
        // Collapse types
        const collapses = result.content.filter((item) => item.type === "collapse");
        expect(collapses.some((c) => c.size === 1)).toBe(true);
        expect(collapses.some((c) => c.size === 2)).toBe(true);
        expect(collapses.some((c) => !c.size)).toBe(true); // Simple collapse
        // Content verification
        const jsonStr = JSON.stringify(result);
...
//# sourceMappingURL=samples-individual.test.js.map