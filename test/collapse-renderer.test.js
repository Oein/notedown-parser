...
describe("Collapse Renderer with Size", () => {
    ...
    test("should include size-specific CSS styles", () => {
        ...
        expect(styleContent).toContain("border-left: 4px solid");
    });
    test("should render collapse with proper nesting structure", () => {
        ...
        expect(detailsElement?.classList.contains("notedown-collapse-2")).toBe(true);
        expect(detailsElement?.getAttribute("data-size")).toBe("2");
        const summaryElement = detailsElement?.querySelector("summary");
        expect(summaryElement).toBeTruthy();
        expect(summaryElement?.classList.contains("notedown-collapse-title")).toBe(true);
        expect(summaryElement?.textContent?.trim()).toBe("Test Collapse");
        const contentElement = detailsElement?.querySelector(".notedown-collapse-content");
        expect(contentElement).toBeTruthy();
    });
});
//# sourceMappingURL=collapse-renderer.test.js.map