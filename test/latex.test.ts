...
function isTextNode(node: NotedownInlineContent): node is NotedownTextNode {
  return "text" in node;
}

// Type guard for formatted nodes
function isFormattedNode(
  node: NotedownInlineContent
): node is NotedownFormattedNode {
...
describe("LaTeX Formula Tests", () => {
  it("should correctly parse inline LaTeX formulas", () => {
    ...
    const paragraph = parsed.content[0] as NotedownParagraph;
    const textContent = paragraph.content[0] as NotedownTextContent;
  ...
  it("should handle dollar signs", () => {
    ...
    const latexNode = textContent.content.find(
      (node) => isFormattedNode(node) && node.format === "latex"
    );
  ...
  it("should load and parse the LaTeX sample document", () => {
    const ndContent = fs.readFileSync(
      path.join(__dirname, "../samples/latex.nd"),
      "utf8"
    );
