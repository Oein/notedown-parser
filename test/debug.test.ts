...
import type { NotedownContentItem } from "../src/types";
...
describe("Debug Test", () => {
  it("should parse a simple title correctly", () => {
    ...
    const foundTitle = parsed.content.some((node: any) => {
      if (node.type === "paragraph" && Array.isArray(node.content)) {
        return node.content.some((item: any) => item && item.type === "title");
      }
      return false;
    });
  ...
  it("should parse a title with meta correctly", () => {
    ...
    const hasTitle = parsed.content.some((node: any) => {
      if (node.type === "paragraph" && Array.isArray(node.content)) {
        return node.content.some((item: any) => item && item.type === "title");
      }
      return false;
